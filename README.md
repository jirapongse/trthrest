...
```
To use this **odata** tag, the custom JSON marshaller is defined for this type. The custom marshaller will be called when **json.Marshal** method is called with the defined type as an argument, such as **TickHistoryMarketDepthExtractionRequest**.  
```
...
After the HTTP request is sent, the HTTP response will return with the JSON object. To decode the returned JSON object, **json.Unmarshal** function is called with **RawExtractionResult** type as an argument.
